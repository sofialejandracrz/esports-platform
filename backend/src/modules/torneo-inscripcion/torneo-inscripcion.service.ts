...
export class TorneoInscripcionService {
  ...
  async create(createTorneoInscripcionDto: CreateTorneoInscripcionDto): Promise<TorneoInscripcion> {
    const { torneoId, usuarioId, estadoId, ...inscripcionData } = createTorneoInscripcionDto;
    ...
    if (!estado) {
      throw new BadRequestException(`Estado de inscripción con ID ${estadoId} no encontrado`);
    }

    const inscripcionExistente = await this.torneoInscripcionRepository.findOne({
      where: {
        torneo: { id: torneoId },
  ...
  async update(id: string, updateTorneoInscripcionDto: UpdateTorneoInscripcionDto): Promise<TorneoInscripcion> {
    ...
    const { torneoId, usuarioId, estadoId, ...inscripcionData } = updateTorneoInscripcionDto;
    ...
    Object.assign(torneoInscripcion, inscripcionData);
    return await this.torneoInscripcionRepository.save(torneoInscripcion);
  }
